...
class CuartoBimestreViewModel with ChangeNotifier {
  ...
  final CuartoBimestreModel _model = CuartoBimestreModel.defaultModel();
  ...
  List<AsistenciaEstudianteCuarto> get filteredEstudiantes => _filteredEstudiantes;
  ...
  Future<void> _cargarDatosDesdeSQLite() async {
    try {
      ...
      final result = await _databaseHelper.rawQuery('''
        ...
        WHERE periodo_id = ? AND materia_id LIKE '%cuarto_bimestre%'
      ''', [_model.bimestre.id]);
  ...
  Future<void> _insertarDatosEjemploEnSQLite() async {
    try {
      for (int i = 1; i <= 60; i++) {
        final estudiante = AsistenciaEstudianteCuarto(
          item: i,
        ...
        ''', [
          'asist_cuarto_${estudiante.item}',
          'est_${estudiante.item}',
          _model.bimestre.id,
          'materia_cuarto_bimestre',
          0,
      ...
      print('✅ ${_model.estudiantes.length} estudiantes insertados en SQLite para cuarto bimestre');
    } catch (e) {
    ...
    try {
      ...
      for (var row in result) {
        try {
          ...
          final estudiante = AsistenciaEstudianteCuarto.fromMap(Map<String, dynamic>.from(datos));
          _model.estudiantes.add(estudiante);
        } catch (e) {
      ...
      print('✅ ${_model.estudiantes.length} estudiantes cargados desde SQLite para cuarto bimestre');
    } catch (e) {
      print('❌ Error cargando datos desde SQLite: $e');
      _cargarDatosEjemplo();
    }
  ...
  Future<void> exportarACSV() async {
    try {
      ...
      for (var estudiante in _filteredEstudiantes) {
        ...
        row.addAll([
          estudiante.octL, estudiante.octM, estudiante.octMi, estudiante.octJ, estudiante.octV,
          estudiante.novL, estudiante.novM, estudiante.novMi, estudiante.novJ, estudiante.novV,
          estudiante.dicL, estudiante.dicM, estudiante.dicMi, estudiante.dicJ, estudiante.dicV,
        ]);
